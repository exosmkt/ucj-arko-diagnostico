...
Devolva um relatório estruturado EXCLUSIVAMENTE em JSON, com EXATAMENTE este formato:
{
  ...
  "dashboard": {
    "receita_pj_por_fonte": [ { "fonte": "ex: Consultas", "valor": <number puro> } ],
    "despesas_pj": [ { "categoria": "ex: Aluguel da clínica", "valor": <number puro>, "tipo": "clinica" } ],
    "distribuicao_socios": [ { "socio": "nome", "cotas_pct": <number>, "valor_recebido": <number puro> } ],
    "pf_saidas_por_categoria": [ { "categoria": "ex: Moradia", "valor": <number puro> } ]
  }
}
Sobre o "dashboard" (use APENAS números puros, sem "R$" nem pontos de milhar, extraídos dos documentos):
- "despesas_pj[].tipo" = "clinica" para custos legítimos da operação, ou "pessoal" para despesas pessoais
  lançadas indevidamente na PJ (ex.: festa de família, estética pessoal, viagem de turismo). NÃO inclua
  distribuição de lucros nas despesas_pj (ela não é despesa).
- "distribuicao_socios": quanto cada sócio(a) recebeu de distribuição no período + o % de cotas que detém.
- "pf_saidas_por_categoria": agrupe as saídas do extrato PF em 4-7 categorias (Moradia, Filhos/Escola,
  Investimentos, Dívidas/Cartão, Pessoal/Lazer, etc.).
